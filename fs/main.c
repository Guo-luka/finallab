...
PUBLIC void task_fs() {
	...
	//int src = fs_msg.source;	/* caller proc nr. */
		...
		phys_copy((void*)va2la(TASK_FS, pathname),
			  ...
			  name_len);
		pathname[name_len] = 0;
		syslog("process %d %s %s ", src, msg_name[msgtype], pathname);
		break;}
            case CLOSE:{
		//char pathname[MAX_PATH];
		//int name_len = fs_msg.NAME_LEN;	/* length of filename */
		//printl("here77 %d", name_len);
	//int src = fs_msg.source;	/* caller proc nr. */
		//assert(name_len < MAX_PATH);
		//phys_copy((void*)va2la(TASK_FS, pathname),
			//  (void*)va2la(src, fs_msg.PATHNAME),
			 // name_len);
		//pathname[name_len] = 0;
		syslog("process %d %s ", src, msg_name[msgtype]);
		break;}
            case READ:
		//int fd = fs_msg.FD;
		//struct inode* pin = proc_table[src]->filp[fd]->fd_inode;
		syslog("process %d %s ", src, msg_name[msgtype]);
		break;
            case WRITE:
		syslog("process %d %s ", src, msg_name[msgtype]);
