...
PUBLIC int do_open()
{
	...
	printl("here: inode_nr: %d flags: %d path: %s", inode_nr, flags, pathname);
	...
	//    struct inode* q = 0;
		printl("\n");
		   for (p = &inode_table[0]; p < &inode_table[8]; p++) {
	       		printl(" %d %d",p->i_num, p->i_cnt);
		    }		
...
PRIVATE void new_dir_entry(struct inode *dir_inode,int inode_nr,char *filename)
{
	...
	//dir_inode->i_cnt += 1;
	//get_inode(dir_inode->i_dev, inode_nr);
	...
	struct inode* pp;
	for (pp = &inode_table[0]; pp < &inode_table[8]; pp++) {
	       		printl(" %d %d",pp->i_num, pp->i_cnt);
		    }
}
